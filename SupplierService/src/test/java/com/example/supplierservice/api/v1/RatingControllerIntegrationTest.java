...
class RatingControllerIntegrationTest {
    ...
    @Transactional
    @SneakyThrows
    void whenCreateRatingThenReturnProductRatingListOneMore() {
        Product product = service.findById(1L);

        assertEquals(4, product.getRatings().size());

        RatingDTO ratingExpected = RatingDTO
                .builder()
                .rating(5)
                .productId(1L)
                .build();
        ...
        assertEquals(5, product.getRatings().size());
